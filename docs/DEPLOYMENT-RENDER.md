...
Đây là phương án trả phí dự phòng. Phương án hiện tại được chọn là [Cloudflare Free](../DEPLOYMENT.md). Blueprint Render được lưu tại `deploy/render.paid.yaml`, không phải cấu hình mặc định.
...
2. Chọn **New → Blueprint** và chọn repository **`huu7725/candate`**, nhánh **`main`**. Đường dẫn Blueprint: `deploy/render.paid.yaml`.
3. Render đọc cấu hình trong [render.paid.yaml](../deploy/render.paid.yaml): Web Service Node.js, Singapore, disk 1 GB, build/test, start và health check.
